...
class MockCollection:
    ...
    async def count_documents(self, query):
        ...
        # Simple count for empty query or specific matches
        count = 0
        for item in self.data:
            if all(item.get(k) == v for k, v in query.items()):
                count += 1
...
# Try to connect to Mongo, fall back to Mock if fails
try:
    logger.info(f"Connecting to MongoDB at {mongo_url}...")
    # Set a short timeout to fail fast if no mongo
    client = AsyncIOMotorClient(mongo_url, serverSelectionTimeoutMS=2000)
    # Trigger connection check (async check would happen on first query, but we can assume it works or rely on error handling later)
    # Ideally we'd await client.server_info() but we are in top level.
    # Let's verify connection lazily, or just use Mock if user requested.
    # For this specific user case: FORCE MOCK if it fails or by default since they have issues.
...
# Override for local testing without mongo (COMMENT OUT TO USE REAL MONGO)
# client = MockClient(mongo_url)
# db = client[os.environ.get('DB_NAME', 'dental_clinic')]
    ...
    async def broadcast(self, message: dict):
        for connection in self.active_connections:
            try:
                ...
                # but let's leave it for the disconnect handler for now
                pass
...
async def login(credentials: UserLogin):
    ...
    return {
        "access_token": access_token,
        "token_type": "bearer",
        "user": UserResponse(**{k: user[k] for k in ["id", "name", "cpf", "birth_date", "created_at"]})
    }
...
async def get_me(current_user: dict = Depends(get_current_user)):
    ...
    return UserResponse(**{k: current_user[k] for k in ["id", "name", "cpf", "birth_date", "created_at"]})
...
async def create_staff(staff_data: StaffCreate, current_user: dict = Depends(get_staff_user)):
    ...
    await manager.broadcast({"type": "staff_updated"})
...
async def create_doctor(doctor_data: DoctorCreate, current_user: dict = Depends(get_staff_user)):
    ...
    await manager.broadcast({"type": "doctors_updated"})
...
async def create_appointment(appointment: AppointmentCreate, current_user: dict = Depends(get_current_user)):
    ...
    # Convert datetime to string for JSON serialization if needed, but dict has datetime objects
    # Fastapi handles return, but for manual json dump we might need conversion
    # However, AppointmentResponse handles it. ConnectionManager.broadcast uses send_json.
    # We should convert datetime objects in appointment_dict to strings or rely on a custom encoder if send_json supports it.
    # Standard json.dumps doesn't support datetime.
    # Let's simple send a notification payload with ID and basic info to trigger refresh
    await manager.broadcast({
        "type": "new_appointment", 
        "data": {
            "id": appointment_id,
            "patient": current_user.get("name", ""),
            "date": appointment.date,
            "time": appointment.time
        }
    })
...
async def cancel_appointment(appointment_id: str, current_user: dict = Depends(get_current_user)):
    ...
    await manager.broadcast({"type": "appointment_cancelled", "id": appointment_id})
...
async def update_appointment(appointment_id: str, data: AppointmentUpdate, current_user: dict = Depends(get_staff_user)):
    ...
    if 'completed_at' in broadcast_data and isinstance(broadcast_data['completed_at'], datetime):
        broadcast_data['completed_at'] = broadcast_data['completed_at'].isoformat()
        
    await manager.broadcast({"type": "appointment_updated", "data": broadcast_data})
        ...
        try:
            ...
            if len(date_parts) == 3:
                ...
                if apt_month == target_month and apt_year == target_year:
                    monthly_total += apt.get("paid_value", 0)
...
async def get_all_patients(current_user: dict = Depends(get_staff_user)):
    ...
    return [UserResponse(**{k: p[k] for k in ["id", "name", "cpf", "birth_date", "created_at"]}) for p in patients]
...
async def get_patient(patient_id: str, current_user: dict = Depends(get_staff_user)):
    ...
    return {
        "patient": UserResponse(**{k: patient[k] for k in ["id", "name", "cpf", "birth_date", "created_at"]}),
        "appointments": [AppointmentResponse(**a) for a in appointments]
    }
