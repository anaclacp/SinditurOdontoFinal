...
export default function AppointmentsScreen() {
  ...
  const loadBookedSlots = async (dateStr: string) => {
    ...
    try {
      const brDate = formatDateToBrazilian(dateStr);
      const response = await appointmentsAPI.getBookedSlots(selectedDoctor.id, brDate);
  ...
  const handleDateSelect = (day: any) => {
    ...
    loadBookedSlots(day.dateString);
  };

  const isTimePast = (time: string): boolean => {
    // Check if the selected date+time is in the past (Brazil UTC-3)
  ...
  const getTimeSlotState = (time: string): 'available' | 'booked' | 'past' => {
    if (isTimeBooked(time)) return 'booked';
    if (isTimePast(time)) return 'past';
    return 'available';
  };
  ...
  const renderTimeSlot = (time: string) => {
    const state = getTimeSlotState(time);
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          ...
          state === 'booked' && styles.timeSlotBooked,
          state === 'past' && styles.timeSlotPast,
        ]}
        onPress={() => {
          if (!isDisabled) {
      ...
      >
        <Text
          style={[
            ...
            state === 'booked' && styles.timeTextBooked,
            state === 'past' && styles.timeTextPast,
          ]}
        >
          {state === 'booked' ? 'Ocupado' : time}
        </Text>
        {state === 'booked' && (
          <Ionicons name="close-circle" size={14} color="#F44336" style={{ marginTop: 2 }} />
        )}
        {state === 'past' && (
          <Ionicons name="time-outline" size={14} color="#999" style={{ marginTop: 2 }} />
        )}
      </TouchableOpacity>
    );
  };
          ...
          <>
            ...
            <View style={styles.legendContainer}>
              <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: '#FFFFFF', borderColor: '#E0E0E0', borderWidth: 1 }]} />
              ...
              <View style={styles.legendItem}>
                ...
                <Text style={styles.legendText}>Passado</Text>
              </View>
            </View>
